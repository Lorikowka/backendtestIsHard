...
test('discard endpoint releases a pending payment reservation with a valid token', async () => {
  await withServer(async baseUrl => {
    const unique = Date.now().toString(36);
    const paymentId = `pay_discard_${unique}`;
    const statusToken = `discard-token-${unique}`;

    await db.createPayment({
      id: paymentId,
      order_id: `order_discard_${unique}`,
      amount: 3000,
      currency: 'RUB',
      status: 'pending',
      description: 'Discard pending reservation test',
      customer_email: 'discard-test@example.com',
      customer_phone: '+7 999 111-22-33',
      customer_name: 'Discard Test',
      service_id: 'consult-online',
      service_name: 'Individual online consultation',
      payment_method: 'test',
      metadata: {
        statusToken,
        serviceId: 'consult-online',
        serviceName: 'Individual online consultation',
        sessionDate: '2099-01-01',
        sessionTime: '10:00',
        sessionDatetime: '2099-01-01T10:00:00'
      }
    });

    const response = await fetch(`${baseUrl}/api/payments/${encodeURIComponent(paymentId)}/discard`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
