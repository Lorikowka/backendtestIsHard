...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // или если origin совпадает с разрешенными/localhost
...
app.post('/api/payments/:id/discard',
  ...
  async (req, res) => {
    try {
      ...
      let providerStatusChecked = MOCK_MODE;
      if (payment.status === 'pending' && !MOCK_MODE) {
        try {
          const checkController = new AbortController();
          const checkTimeout = setTimeout(() => checkController.abort(), 5000);
      ...
      if (payment.status === 'pending' && providerStatusChecked) {
    ...
    try {
      ...
      if (['SLOT_FULL', 'SLOT_TAKEN', 'DUPLICATE_GROUP_BOOKING', 'SQLITE_CONSTRAINT'].includes(error.code)) {
        ...
        return res.status(409).json({
          success: false,
          error: isDuplicate 
            ? 'Вы уже записаны на этот групповой тренинг.' 
            : 'Выбранное время уже занято. Пожалуйста, выберите другой слот.',
          code: error.code
        });
      }
...
app.get('/api/schedule', async (req, res) => {
  try {
    ...
    // Определяем "сегодня" в часовом поясе Москвы (UTC+3)
    // Это гарантирует консистентность дат независимо от настроек времени на сервере
